...
export default class GLTF_utils {
    ...
    public static async get_all_meshes(content: GLTF): Promise<Mesh[]> {
        ...
        if (content) {
            ...
            console.error("SVELTHREE > GLTF_utils > async 'get_all_meshes(content)' -> 'content' not available!", { content })
        }
    ...
    public static async get_all_scene_meshes(scene: Group): Promise<Mesh[]> {
        ...
        if (scene) {
            ...
            console.error("SVELTHREE > GLTF_utils > async 'get_all_scene_meshes(scene)' -> 'scene' not available!", { scene })
        }
    ...
    public static async get_all_lights(content: GLTF): Promise<Light[]> {
        ...
        if (content) {
            ...
            console.error("SVELTHREE > GLTF_utils > async 'get_all_lights(content)' -> 'content' not available!", { content })
        }
    ...
    public static async get_all_scene_lights(scene: Group): Promise<Light[]> {
        ...
        if (scene) {
            ...
            console.error("SVELTHREE > GLTF_utils > async 'get_all_scene_lights(scene)' -> 'scene' not available!", { scene })
        }
        ...
        if (scene) {
            ...
            const check_obj = (obj: Object3D, check: string, depot: (Mesh | Light | Camera)[]) => async () => {
                if(obj) {
                    ...
                    console.error("SVELTHREE > GLTF_utils > async 'get_all(scene, ...)' -> async check_obj(obj, ...) -> 'obj' not available!", { obj })
                }
            }
