...
export class XRHandTouchSphereExt extends XRHandTouch {
    ...
    //we don't need this!
    performInsideCheck(joint: Group, raycaster: Raycaster) {


        // do this only is sphere is inside, break it as soon it intersects
        // do we need this? at all? (double)
        if (joint.userData.touchInside) {

            const testRaycasterDir = new Vector3().subVectors(joint.userData.lastOrigin, joint.userData.origin).normalize()
            const testRaycasterLength = joint.userData.origin.distanceTo(joint.userData.lastOrigin)
        ...
        // cast a ray from joint.userData.origin --> joint.userData.lastOrigin
     ...
     */
    // SPHERE CLEANUP: we don't need this at all here, we're untouching if there are no intersection-indices in the update function
    /*
