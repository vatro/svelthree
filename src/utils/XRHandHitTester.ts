        ...
        updatePinchRay(
            ...
            if(!hand.userData.lastPinchRayDirection) {
                ...
                if(originXR2a_world.distanceTo(originXR2b_world) < 0.05) {
                    let lerpedDirection: Vector3 = new Vector3().lerpVectors(hand.userData.lastPinchRayDirection, directionXR.normalize(), 0.1)
                    let lerpedPinchRayPoint: Vector3 = new Vector3().lerpVectors(hand.userData.lastPinchRayPoint, target, 0.1)
                    ...
                    let lerpedDirection: Vector3 = new Vector3().lerpVectors(hand.userData.lastPinchRayDirection, directionXR.normalize(), 0.5)
                    let lerpedPinchRayPoint: Vector3 = new Vector3().lerpVectors(hand.userData.lastPinchRayPoint, target, 0.5)
                    directionXR = lerpedDirection
                    pinchRayPoint = lerpedPinchRayPoint
                    hand.userData.lastPinchRayDirection = directionXR.clone()
                    hand.userData.lastPinchRayPoint = lerpedPinchRayPoint.clone()
            ...
            if (intersections.length > 0) {
                ...
                switch(hand.userData.pinchConfig.mode) {
                    case "remote":
                        if (intersections.length > 0) {
                            if(intersections[0].object.userData.svelthreeComponent.pinchRemoteEnabled() ||
                    ...
                    case "touch":
                        if (intersections.length > 0) {
                            if(intersections[0].object.userData.svelthreeComponent.pinchTouchEnabled() ||
                               intersections[0].object.userData.svelthreeComponent.pinchHybridEnabled()
                                ) {
                                this.raycasterRay.showPinchRay(
                                rayData,
                    ...
                    case "hybrid":
                        if (intersections.length > 0) {
                            ...
                            if (intersections[0].distance <= (pinchRayLength + hand.userData.pinchConfig.distance.touch)) {
                                ...
                                else if (intersections[0].object.userData.svelthreeComponent.pinchRemoteEnabled() ||
                                    intersections[0].object.userData.svelthreeComponent.pinchHybridEnabled()) {
                                    this.raycasterRay.showPinchRay(
                                    rayData,
                                    hand.userData.pinchConfig.materials.remote,
