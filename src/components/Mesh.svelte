...
<script lang="ts">
	...
	$: if (shadow_root_el && mesh && !shadow_dom_el) create_shadow_dom_target()
	...
	async function create_shadow_dom_target() {
		if (browser) {
			...
			shadow_dom_el.dataset.kind = "Mesh"
			...
			if (shadow_target) {
				...
				console.error(
					"SVELTHREE > Mesh > create_shadow_dom_target > Wasn't able to append shadow DOM element, no 'shadow_target'!",
					{ shadow_target },
