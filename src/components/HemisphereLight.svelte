...
<script lang="ts">
	...
	/** Returns the `light` instance created by the component & allows providing (_injection_) of (_already created / premade_) `THREE.HemisphereLight` instances. */
	export let light: HemisphereLight = undefined
	...
	function on_instance_provided(): void {
		...
		if (light.type === "HemisphereLight") {
			...
			throw new Error(
				`SVELTHREE > HemisphereLight Error: provided 'light' instance has wrong type '${light.type}', should be 'HemisphereLight'!`
			)
		}
	}
	...
	// GENERATOR REMARK: 'reactive_re_creation_logic' not implemented for 'HemisphereLight'!

	/** Initializes `HemisphereLight` with provided constructor parameters.*/
	export let params: ConstructorParameters<typeof HemisphereLight> = undefined

	$: if (!light && create) {
		if (params) {
			light = new HemisphereLight(...params)
		} else {
	...
	async function create_shadow_dom_target() {
		if (browser) {
			...
			shadow_dom_el.dataset.kind = "HemisphereLight"
	...
	export let props: { [P in keyof HemisphereLightProps]: HemisphereLightProps[P] } = undefined
	...
	function add_helper(): void {
		if (helperParams) {
			light.userData.helper = new HemisphereLightHelper(light, ...helperParams)
		} else {
			light.userData.helper = new HemisphereLightHelper(light, 1)
		}
	...
	export const get_helper = (): HemisphereLightHelper => light.userData.helper as HemisphereLightHelper
	...
	export const get_instance = (): HemisphereLight => light
