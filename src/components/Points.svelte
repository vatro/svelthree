<script lang="ts">
    ...
    function onPointsProvided(): void {
        ...
        if (points.type === "Points") {
            ...
            if (points.material) {
                ...
                console.warn(
                    "SVELTHREE > Points : Points provided, but has no material!",
                    { points: points }
                )
            }
    ...
    function checkParentSlot() {
        if (generate) {
            if (points && !parent) {
                ...
                if (!points) {
                    console.error(
                        "SVELTHREE > Points : 'parent' check : no points provided yet!"
                    )
                } else if (parent) {
    ...
    $: if (geometry && material && !points && generate) {
        ...
        if (logInfo)
            console.info(
                "SVELTHREE > Points : " + geometry.type + " created!",
                {
                    points: points
                }
            )
        if (logInfo)
            console.info(
                "SVELTHREE > Points : saved 'geometry' (generated):",
                geometry
            )
        if (logInfo)
            console.info(
                "SVELTHREE > Points : saved 'material' (generated):",
                material
            )
    }
    ...
    $: points
        ? tryAddingPoints()
        : console.error("SVELTHREE > Points : points was not created!")
    ...
    */
    ...
    $: if (useBVH && geometry) {
        console.log(
            "%c--------> Points > reactive statement! useBVH && geometry",
            `${css_rs}`,
            useBVH,
            geometry
        )
        ...
        if (
            Object.keys(BufferGeometry.prototype).indexOf("computeBoundsTree") >
            -1
        ) {
            if (logInfo)
                console.info(
                    "SVELTHREE > Points : Using BVH, points.matrixWorld: ",
                    points.matrixWorld
                )
            ...
            if (logInfo)
                console.info(
                    "SVELTHREE > Points : (BVH) computeBoundsTree finished, points: ",
                    points
                )
        } else {
            console.error(
                "SVELTHREE > Points : (BVH) points.geometry.computeBoundsTree not available!"
            )
        }
        //}
        ...
        if (points.geometry.hasOwnProperty("boundsTree")) {
            if (logInfo)
                console.info(
                    "SVELTHREE > Points : (BVH) try disposing boundsTree!",
                    points
                )
            if (points.geometry.hasOwnProperty("disposeBoundsTree")) {
                ...
                if (logInfo)
                    console.info(
                        "SVELTHREE > Points : (BVH) boundsTree disposed!",
                        points
                    )
            } else {
                console.error(
                    "SVELTHREE > Points : (BVH) points.geometry.disposeBoundsTree not available!"
                )
            }
        }
    }
    ...
    function updateProps() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! props",
                `${css_rs}`,
                props
            )
        sProps.update(props)
    ...
    */
    ...
    function setPositionFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! pos",
                `${css_rs}`,
                pos
            )
    ...
    function setRotationFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! rot",
                `${css_rs}`,
                rot
            )
    ...
    */
    ...
    function setQuaternionFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! quat",
                `${css_rs}`,
                quat
            )
    ...
    function setScaleFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! scale",
                `${css_rs}`,
                scale
            )
    ...
    */
    ...
    function setLookAtFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! lookAt",
                `${css_rs}`,
                lookAt
            )
    ...
    function setMatrixFromValue() {
        if (logRS)
            console.log(
                "%c--------> Points > reactive statement! matrix",
                `${css_rs}`,
                matrix
            )
        ...
        fnOnMount
            ...
            : () => {
                  if (parent) {
                      if (logInfo)
                          console.info(
                              "SVELTHREE > onMount : Points, parent: ",
                              parent
                          )
                  } else {
                  ...
                  return () => {
                      if (logInfo)
                          console.info("SVELTHREE > onDestroy : Points")
