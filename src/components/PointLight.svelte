...
<script lang="ts">
	...
	export let light: PointLight = undefined
	...
	function on_instance_provided(): void {
		...
		if (light.type === "PointLight") {
			...
			throw new Error(
				`SVELTHREE > PointLight Error: provided 'light' instance has wrong type '${light.type}', should be 'PointLight'!`
			)
		}
	}
	...
	export let params: ConstructorParameters<typeof PointLight> = undefined

	$: if (!light && create) {
		if (params) {
			light = new PointLight(...params)
		} else {
	...
	async function create_shadow_dom_target() {
		if (browser) {
			...
			shadow_dom_el.dataset.kind = "PointLight"
	...
	export const get_instance = (): PointLight => light
