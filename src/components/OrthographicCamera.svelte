   ...
   */
	...
	async function create_shadow_dom_target() {
		if (browser) {
			...
			shadow_dom_el.dataset.kind = "OrthographicCamera"
			...
			if (shadow_target) {
				...
				console.error(
					"SVELTHREE > OrthographicCamera > create_shadow_dom_target > Wasn't able to append shadow DOM element, no 'shadow_target'!",
					{ shadow_target },
