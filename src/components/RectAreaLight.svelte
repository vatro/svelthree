...
<script lang="ts">
	...
	async function create_shadow_dom_target() {
		if (browser) {
			...
			shadow_dom_target.dataset.kind = "RectAreaLight"
			...
			if (shadow_target) {
				...
				console.error(
					"SVELTHREE > RectAreaLight > create_shadow_dom_target > Wasn't able to append shadow DOM element, no 'shadow_target'!",
					{ shadow_target },
