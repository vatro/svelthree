...
<script lang="ts">
	...
	import type { PropsMesh, PropMat } from "../types/types-comp-props.js"
	import type { RemoveFirst, MeshAssignableMaterial } from "../types/types-extra.js"
	...
	export let mesh: THREE_Mesh | undefined | null = undefined
	let mesh_uuid: string | undefined | null = undefined
	...
	let extracted_material: MeshAssignableMaterial | undefined = undefined
	export let material: AssignedMaterial | undefined | null = undefined
	let material_ref: MeshAssignableMaterial | undefined = undefined
	...
	export let params: ConstructorParameters<typeof THREE_Mesh> | undefined = undefined
	...
	$: if (!mesh && create) {
		if (geometry && material) {
			...
			if (geometry && verbose && log_dev) console.debug(...c_dev(c_name, `${geometry.type} created!`, { mesh }))
		}
	}
	...
	function handle_instance_change(): void {
		if (mesh) {
			...
			if (our_parent) our_parent.add(mesh)
			mesh_uuid = mesh.uuid
	...
	export let props: PropsMesh | undefined = undefined
	...
	function set_pos() {
		...
		if (mesh && pos) {
			...
			console.error(`SVELTHREE > ${c_name} > set_pos : invalid 'mesh' instance and / or 'pos' value!`, {
				mesh,
				pos
			})
		}
	...
	function set_rot() {
		...
		if (mesh && rot !== undefined) {
			PropUtils.setRotationFromValue(mesh, rot)
		} else {
			console.error(`SVELTHREE > ${c_name} > set_rot : invalid 'mesh' instance and / or 'rot' value!`, {
				mesh,
				rot
			})
		}
	...
	function set_quat() {
		...
		if (mesh && quat) {
			...
			console.error(`SVELTHREE > ${c_name} > set_quat : invalid 'mesh' instance and / or 'quat' value!`, {
				mesh,
				quat
			})
		}
	...
	function set_scale() {
		...
		if (mesh && scale !== undefined) {
			...
			console.error(`SVELTHREE > ${c_name} > set_scale : invalid 'mesh' instance and / or 'scale' value!`, {
				mesh,
				scale
			})
		}
	...
	function set_lookat() {
		...
		if (mesh && lookAt) {
			...
			console.error(`SVELTHREE > ${c_name} > set_lookat : invalid 'mesh' instance and / or 'lookAt' value!`, {
				mesh,
				lookAt
			})
		}
	...
	function set_matrix(): void {
		...
		if (mesh && matrix) {
			...
			console.error(`SVELTHREE > ${c_name} > set_matrix : invalid 'mesh' instance and / or 'matrix' value!`, {
				mesh,
				matrix
			})
		}
	...
	function add_box_helper() {
		if (mesh) {
			...
			console.error(`SVELTHREE > ${c_name} > Cannot add box helper to unavailable 'mesh' instance!`, { mesh })
		}
	}
	...
	$: if (interaction_enabled && raycast && !block && mesh) {
		if (!raycast.includes(mesh)) {
	...
	export const remove_instance_from_parent = async (): Promise<boolean> => {
		if (mesh) {
			...
			console.error(`SVELTHREE > ${c_name} > remove_instance_from_parent : invalid 'mesh' instance value!`, {
				mesh
			})
			return false
		}
	...
	export const get_instance = (): THREE_Mesh | undefined | null => mesh
