...
<script lang="ts">
	...
	import type { PropsGroup } from "../types/types-comp-props.js"
	...
	export let group: THREE_Group | undefined | null = undefined
	let group_uuid: string | undefined | null = undefined
		...
		if (group) {
			if ((group_uuid && group.uuid !== group_uuid) || !group_uuid) {
				...
				if (old_instance) {
					...
					if (props) {
						try {
							sProps = new SvelthreeProps(group)
						} catch (err) {
							console.error(err)
						}
					}
			...
			if (our_parent) our_parent.add(group)
			group_uuid = group.uuid
	...
	export let props: PropsGroup | undefined = undefined
	...
	function set_pos() {
		...
		if (group && pos) {
			...
			console.error(`SVELTHREE > ${c_name} > set_pos : invalid 'group' instance and / or 'pos' value!`, {
				group,
				pos
			})
		}
	...
	function set_rot() {
		...
		if (group && rot !== undefined) {
			PropUtils.setRotationFromValue(group, rot)
		} else {
			console.error(`SVELTHREE > ${c_name} > set_rot : invalid 'group' instance and / or 'rot' value!`, {
				group,
				rot
			})
		}
	...
	function set_quat() {
		...
		if (group && quat) {
			...
			console.error(`SVELTHREE > ${c_name} > set_quat : invalid 'group' instance and / or 'quat' value!`, {
				group,
				quat
			})
		}
	...
	function set_scale() {
		...
		if (group && scale !== undefined) {
			...
			console.error(`SVELTHREE > ${c_name} > set_scale : invalid 'group' instance and / or 'scale' value!`, {
				group,
				scale
			})
		}
	...
	function set_lookat() {
		...
		if (group && lookAt) {
			...
			console.error(`SVELTHREE > ${c_name} > set_lookat : invalid 'group' instance and / or 'lookAt' value!`, {
				group,
				lookAt
			})
		}
	...
	function set_matrix(): void {
		...
		if (group && matrix) {
			...
			console.error(`SVELTHREE > ${c_name} > set_matrix : invalid 'group' instance and / or 'matrix' value!`, {
				group,
				matrix
			})
		}
	...
	function add_box_helper() {
		if (group) {
			...
			console.error(`SVELTHREE > ${c_name} > Cannot add box helper to unavailable 'group' instance!`, { group })
		}
	}
	...
	export const remove_instance_from_parent = async (): Promise<boolean> => {
		if (group) {
			...
			console.error(`SVELTHREE > ${c_name} > remove_instance_from_parent : invalid 'group' instance value!`, {
				group
			})
			return false
		}
	...
	export const get_instance = (): THREE_Group | undefined | null => group
