<script lang="ts">
   ...
   // $: lightHelper && light ? (lightHelper.matrixAutoUpdate = light.matrixAutoUpdate, console.warn("SVELTHREE > SvelthreeLightHelper, matrixAutoUpdate CHANGE! --> light.matrixAutoUpdate:", light.matrixAutoUpdate)) : null
    ...
    export function startHelperUpdating(): void {
        ...
        if (!doUpdateHelper) {
            console.warn("SVELTHREE > SvelthreeLightHelper > startHelperUpdating > lightHelper.matrixAutoUpdate:", lightHelper.matrixAutoUpdate)
            console.warn("SVELTHREE > SvelthreeLightHelper > startHelperUpdating > light.matrixAutoUpdate:", light.matrixAutoUpdate)
            doUpdateHelper = true
